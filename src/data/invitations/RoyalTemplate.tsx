/**
 * RoyalTemplate.tsx — Self-contained Royal Wedding Invitation
 * 
...
import { Heart, Menu, X, ChevronLeft, ChevronRight, ChevronDown, MapPin, ExternalLink, Sun, Moon, Quote, Music, Star, Clock, Wine, Utensils, Send } from 'lucide-react';
...
const TelegramCTA = ({ link }: { link: string }) => (
  <section className="wedding-section" id="telegram">
    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="max-w-lg mx-auto text-center glass-card p-10">
...
const FAQAccordionItem = ({ item }: { item: FAQItem }) => {
  ...
  return (
    <div className="glass-card px-6 mb-3">
      <button onClick={() => setOpen(!open)} className="w-full flex items-center justify-between py-4 font-display text-base text-foreground text-left">
        {item.question}
...
const RoyalTemplate = () => {
  ...
  return (
    <div id="template-root" className="theme-royal bg-background text-foreground min-h-screen transition-colors duration-500">
      <WeddingNav items={navItems} coupleNames={`${data.couple.bride} & ${data.couple.groom}`} />
      <ThemeToggle defaultDark={true} />
      <HeroSection bride={data.couple.bride} groom={data.couple.groom} date={data.date} backgroundImage={data.gallery[0]} variant="dark" />
