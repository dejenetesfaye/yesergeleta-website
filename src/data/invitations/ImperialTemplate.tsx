...
const AboutUs = ({ couple }: { couple: WeddingData['couple'] }) => (
  <section className="wedding-section" id="about">
    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
      <h2 className="wedding-section-title">About Us</h2>
      <p className="wedding-section-subtitle">Two hearts, one love</p>
      <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8">
        <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="glass-card p-8 text-center">
          {couple.bridePhoto && <div className="w-32 h-32 rounded-full mx-auto mb-4 overflow-hidden border-2 border-primary/30"><img src={couple.bridePhoto} alt={couple.bride} className="w-full h-full object-cover" /></div>}
          <h3 className="font-display text-2xl text-foreground">{couple.bride}</h3>
          <p className="text-primary font-sans text-xs uppercase tracking-widest mt-1 mb-3">The Bride</p>
          {couple.aboutBride && <p className="font-serif text-sm text-muted-foreground italic">{couple.aboutBride}</p>}
        </motion.div>
        <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="glass-card p-8 text-center">
          {couple.groomPhoto && <div className="w-32 h-32 rounded-full mx-auto mb-4 overflow-hidden border-2 border-primary/30"><img src={couple.groomPhoto} alt={couple.groom} className="w-full h-full object-cover" /></div>}
          <h3 className="font-display text-2xl text-foreground">{couple.groom}</h3>
          <p className="text-primary font-sans text-xs uppercase tracking-widest mt-1 mb-3">The Groom</p>
          {couple.aboutGroom && <p className="font-serif text-sm text-muted-foreground italic">{couple.aboutGroom}</p>}
        </motion.div>
      </div>
    </motion.div>
  </section>
...
const VideoSection = ({ url }: { url: string }) => (
  <section className="wedding-section" id="video">
    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
      <h2 className="wedding-section-title">Our Video</h2>
      <p className="wedding-section-subtitle">A glimpse into our journey</p>
      <div className="max-w-6xl mx-auto rounded-xl overflow-hidden shadow-2xl">
        <div className="aspect-video">
          <iframe src={url} width="100%" height="100%" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen title="Wedding video" className="border-0" />
        </div>
      </div>
    </motion.div>
  </section>
  ...
  return (
    <section className="wedding-section" id="gallery">
      <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
        ...
        <div className={`grid ${colClass} gap-3 md:gap-4 max-w-screen-2xl mx-auto`}>
          {images.map((img, i) => (
...
const LoveStory = ({ story }: { story: LoveStoryItem[] }) => (
  <section className="wedding-section" id="love-story">
    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
      <h2 className="wedding-section-title">Our Love Story</h2>
      <p className="wedding-section-subtitle">How it all began</p>
      <div className="max-w-6xl mx-auto space-y-12">
        {story.map((item, i) => (
          <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.15 }} className={`flex flex-col ${i % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'} gap-6 items-center`}>
            {item.image && (
              <div className="w-full md:w-1/2 rounded-xl overflow-hidden aspect-[4/3]">
                <img src={item.image} alt={item.title} loading="lazy" className="w-full h-full object-cover" />
              </div>
            )}
            <div className="w-full md:w-1/2 text-center">
              <span className="text-primary font-sans text-xs font-semibold uppercase tracking-widest">{item.date}</span>
              <h3 className="font-display text-2xl mt-2 mb-3 text-foreground">{item.title}</h3>
              <p className="font-serif text-base text-muted-foreground leading-relaxed">{item.description}</p>
            </div>
          </motion.div>
        ))}
      </div>
...
const TelegramCTA = ({ link }: { link: string }) => (
  <section className="wedding-section" id="telegram">
    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="max-w-3xl mx-auto text-center glass-card p-10">
      <Send size={40} className="text-primary mx-auto mb-4" />
      <h2 className="font-display text-2xl mb-3 text-foreground">Join Our Telegram</h2>
      <p className="font-serif text-base text-muted-foreground italic mb-6">Stay updated with real-time wedding announcements and connect with other guests.</p>
      <a href={link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-8 py-3 rounded-lg gold-gradient-bg text-primary-foreground font-sans font-semibold text-sm hover:opacity-90 transition">
...
const FAQAccordionItem = ({ item }: { item: FAQItem }) => {
  ...
  return (
    <div className="glass-card px-6 mb-3">
      <button onClick={() => setOpen(!open)} className="w-full flex items-center justify-between py-4 font-display text-base text-foreground text-center">
      ...
      <AnimatePresence>
        {open && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
            <p className="font-sans text-sm text-muted-foreground pb-4">{item.answer}</p>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
...
const FAQ = ({ items }: { items: FAQItem[] }) => (
  <section className="wedding-section" id="faq">
    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
      <h2 className="wedding-section-title">FAQ</h2>
      <p className="wedding-section-subtitle">Everything you need to know</p>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
        ...
        <form onSubmit={handleSubmit} className="max-w-3xl mx-auto space-y-5">
          <div className="text-center">
            ...
            <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="w-full px-4 py-3 rounded-lg bg-background border border-border focus:border-primary focus:ring-1 focus:ring-primary outline-none transition font-sans text-foreground text-center" placeholder="Your full name" />
          </div>
          <div className="text-center">
            <label className="block font-sans text-sm font-medium mb-1.5 text-foreground">Phone Number *</label>
            <input type="tel" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} className="w-full px-4 py-3 rounded-lg bg-background border border-border focus:border-primary focus:ring-1 focus:ring-primary outline-none transition font-sans text-foreground text-center" placeholder="Your phone number" />
          </div>
          <div className="text-center">
          ...
          <div className="text-center">
            ...
            <textarea value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} className="w-full px-4 py-3 rounded-lg bg-background border border-border focus:border-primary focus:ring-1 focus:ring-primary outline-none transition font-sans text-foreground resize-none text-center" rows={3} placeholder="A few words for the couple..." />
          </div>
...
const ImperialTemplate = () => {
  ...
  return (
    <div id="template-root" className="theme-imperial bg-background text-foreground min-h-screen transition-colors duration-500">
      ...
      <RSVPForm />
      <div className="wedding-divider" />
      <section className="wedding-section" id="qr-entry">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="max-w-xl mx-auto text-center glass-card p-10">
          <QrCode size={48} className="text-primary mx-auto mb-4" />
          <h2 className="font-display text-2xl mb-3 text-foreground">Event Access</h2>
          <p className="font-serif text-sm text-muted-foreground italic mb-4">Show this QR code at the entrance for quick check-in</p>
          <div className="w-48 h-48 mx-auto bg-foreground/10 rounded-xl flex items-center justify-center border border-border">
            <QrCode size={120} className="text-primary/60" />
          </div>
        </motion.div>
      </section>
